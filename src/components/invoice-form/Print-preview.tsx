...
import { calcRowItem, computeInvoiceTotals } from "@/utils/invoiceCalculations";
...
export default function PrintPreview({ invoice, onClose }: Props) {
  ...
  // === per-row calculation — delegates to shared canonical utility ===
  /**
   * calcRow is a thin wrapper around the shared calcRowItem() so that all
   * call sites in this file continue to work without change.
   * Formula: taxable = base − (base × disc%), gstAmount = taxable × gst%
  ...
  // === totals — delegates to shared canonical utility ===
  /**
   * computeTotals wraps the shared computeInvoiceTotals() so the rest of this
   * component continues to work without changes.
   */
  const computeTotals = (inv: any) => {
    const result = computeInvoiceTotals(inv);
    console.log('📊 FINAL TOTALS RESULT (Shared Utility):', result);
    return result;
  };

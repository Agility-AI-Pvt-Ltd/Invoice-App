...
import { calcRowItem } from "@/utils/invoiceCalculations";
...
export function AddItem({ items: externalItems, setItems: externalSetItems }: Props) {
  ...
  /**
   * Calculate the gross row total using the canonical shared formula:
   *   taxable = base − (base × discount%)
   *   net     = taxable + (taxable × gst%)
   * GST is applied AFTER discount on the taxable value, not on the base.
   */
  const calculateTotal = (item: Item): string => {
    return calcRowItem(item).net.toFixed(2);
  };

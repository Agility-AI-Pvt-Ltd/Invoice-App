...
import { computeFormTotals } from "@/utils/invoiceCalculations";
...
export default function InvoiceForm({ onCancel, initialData }: Props) {
  ...
  /**
   * Compute invoice totals from items using the canonical shared formula.
   * Replaces old local computeTotals() which had two bugs:
   *   - treated discount as an absolute ₹ amount instead of a %
   *   - applied GST on the pre-discount base, not the taxable value
   *   - hardcoded igst = 0 regardless of states
   */
  const computeTotals = (inv: InvoiceModel) => computeFormTotals(inv as any);
  ...
  */
  ...
  const sanitizePayload = (payload: any) => {
    ...
    cleaned.items = (payload.items || []).map((it: any) => {
      ...
      const discountPct = Number(it.discount) || 0;  // always a percentage
      const gst = Number(it.gst) || 0;
      // Correct computation: discount is a %, GST applied after discount
      const base = quantity * unitPrice;
      const discountAmt = base * discountPct / 100;
      const taxable = base - discountAmt;
      const gstAmt = taxable * gst / 100;
      const computedAmount = +(taxable + gstAmt).toFixed(2);
      return {
  ...
  */
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Before save: check inventory availability for items linked to inventory
      // Items manually typed (no inventoryItemId) are skipped — only linekd items are checked
      const availabilityCheckItems = (candidate.items || [])
        .filter((it: any) => it.inventoryItemId && Number(it.inventoryItemId) > 0 && Number(it.quantity) > 0)
        .map((it: any) => ({
          inventoryItemId: Number(it.inventoryItemId),
          quantity: Number(it.quantity),
          name: it.description || 'Unknown item',  // carry name for friendly error display
        }));
      ...
      if (availabilityCheckItems.length > 0) {
        ...
        if (!availability?.success || !availability?.data?.available) {
          // Backend error objects have: { itemId, name, error, requested, available }
          // e.message is undefined — the field is e.error
          const errors = (availability?.data?.errors || []).map((e: any) => {
            const label = e.name || `Item ID ${e.itemId}`;  // use item name; fall back to ID
            const reason = e.error || e.message || 'Insufficient stock';
            return `• ${label}: ${reason} (requested: ${e.requested ?? '?'}, available: ${e.available ?? '?'})`;
          });

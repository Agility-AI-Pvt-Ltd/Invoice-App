...
export default function PricingSection() {
  ...
  return (
    <section
      id="pricing-section"
      ref={sectionRef}
      className="min-h-screen bg-pricing-bg flex items-center justify-center py-12 sm:py-16 md:py-20 px-4 sm:px-6"
    >
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-3 sm:gap-4 mb-6 sm:mb-8">
            <span className={`text-base sm:text-lg ${!isYearly ? 'text-white' : 'text-gray-400'}`}>
              Monthly
            </span>
            <Switch
              checked={isYearly}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <Card
              key={plan.name}
              className={`relative p-6 sm:p-8 transition-all duration-500 hover:scale-105 hover:shadow-2xl ${
                isVisible
            ...
            >
              {plan.popular && (
                <div className="absolute -top-3 sm:-top-4 left-1/2 transform -translate-x-1/2">
                  <span className="bg-gray-800 text-white px-3 sm:px-4 py-1 rounded-full text-xs sm:text-sm font-medium">
                    Popular
                  </span>
                </div>
              )}
              ...
              <div className="text-center mb-8 sm:mb-10">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold text-white-900">{plan.name}</h3>
                </div>
                <p className="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4">{plan.description}</p>
                
                <div className="mb-4 sm:mb-6">
                  <span className="text-3xl sm:text-4xl md:text-5xl font-bold text-white-900">
                    ${isYearly ? plan.yearlyPrice : plan.monthlyPrice}
                  </span>
                  <span className="text-sm sm:text-base text-grey-600 ml-2">
                    /{isYearly ? "year" : "month"}
                  </span>
                </div>
                <p className="text-xs sm:text-sm text-white-600 mb-3 sm:mb-4">{plan.explain}</p>
              </div>
              ...
              <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
                {plan.features.map((feature, featureIndex) => (
                  <div key={featureIndex} className="flex items-center gap-2 sm:gap-3">
                    {feature.included ? (
                      <Check className="w-4 h-4 sm:w-5 sm:h-5 text-pricing-feature-available flex-shrink-0" />
                    ) : (
                      <X className="w-4 h-4 sm:w-5 sm:h-5 text-pricing-feature-unavailable flex-shrink-0" />
                    )}
                    <span
              ...
              <Button
                className={`w-full py-2 sm:py-3 text-xs sm:text-sm font-medium transition-all duration-200 hover:animate-scale-up ${
                  plan.popular
